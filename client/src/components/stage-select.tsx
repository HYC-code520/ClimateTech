...
import { getStageColor, getAllStageColors } from "@/lib/color-schemes";
...
export function StageSelect({ value, onValueChange, placeholder = "Any stage" }: StageSelectProps) {
  const stageOptions = getAllStageColors();
  ...
  return (
    <Select value={value || "all"} onValueChange={(val) => onValueChange(val === "all" ? "" : val)}>
      ...
      <SelectContent className="bg-gray-800 border-gray-600">
        ...
        {stageOptions.map(({ stage, color }) => (
          <SelectItem key={stage} value={stage} className="text-white hover:bg-gray-700 focus:bg-gray-700">
            <div className="flex items-center gap-2">
              <div 
